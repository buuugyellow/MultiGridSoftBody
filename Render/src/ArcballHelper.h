...
namespace Utility
{
    ...
    public:
        ...
        float intersectBox(float mouseX, float mouseY, glm::AABB abox)
        {
            glm::vec3 ray_origin;
            glm::vec3 ray_dir;

            glm::vec2 mouse_pos(mouseX / screen_width, 1.0f - mouseY / screen_height);
            mouse_pos = mouse_pos * 2.0f - glm::vec2(1, 1);
            auto ray = getRay(mouse_pos.x, mouse_pos.y);
            ...
            float t;
            intersectAABB(ray_origin, ray_dir, abox.getMin(), abox.getMax(), t);
            return t;
        };

        float intersectBoxGizmoCube(float mouseX, float mouseY, glm::AABB abox)
        {
            //check testposition if in gizmo cube region?
            mouseY = screen_height - 1 - mouseY;
            if (!(cube_gizmo_region.x < mouseX && mouseX < cube_gizmo_region.x + cube_gizmo_region.z &&
                cube_gizmo_region.y < mouseY && mouseY < cube_gizmo_region.y + cube_gizmo_region.w))
                return -1;
            //mouseY = screen_height - 1 - mouseY;

            glm::vec2 cube_region_mouse(mouseX - cube_gizmo_region.x, mouseY - cube_gizmo_region.y);
            glm::vec3 ray_origin;
            glm::vec3 ray_dir;

            glm::vec2 mouse_pos(cube_region_mouse.x / cube_gizmo_region.z, cube_region_mouse.y / cube_gizmo_region.w);
            mouse_pos = mouse_pos * 2.0f - glm::vec2(1, 1);
            auto ray = getRayCube(mouse_pos.x, mouse_pos.y);
            ray_origin = ray.position;
            ray_dir = ray.direction;
