...
void Renderer::render(GLFWwindow* window, const ViewSettings& view, const SceneSettings& scene)
{
	...
	glEnable(GL_BLEND);
	//glBlendFunci(0, GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA); // accumulation blend target
	...
	else
	{
		...
		glUseProgram(m_sceneobject->m_ssaoProgram);
		...
		auto loc_noise = glGetUniformLocation(m_sceneobject->m_ssaoProgram, "noise");
		glUniform3fv(loc_noise, m_sceneobject->noise.size(), &m_sceneobject->noise[0].x);

		auto loc_samples = glGetUniformLocation(m_sceneobject->m_ssaoProgram, "samples");
		glUniform3fv(loc_samples, m_sceneobject->ssaosamples.size(), &m_sceneobject->ssaosamples[0].x);

		auto loc_campos = glGetUniformLocation(m_sceneobject->m_ssaoProgram, "cam_pos");
		glUniform3fv(loc_campos, 1, &view.camera->eye.x);
		
		auto loc_projection = glGetUniformLocation(m_sceneobject->m_ssaoProgram, "projection");
		glUniformMatrix4fv(loc_projection, 1, false,&projectionMatrix[0][0]);

		auto loc_paras = glGetUniformLocation(m_sceneobject->m_ssaoProgram, "paras");
