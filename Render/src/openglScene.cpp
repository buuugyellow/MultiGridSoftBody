...
void Mesh::drawTransparent(Utility::ArcballCamera* cam) {
	if (!m_draw)
		return;
	m_materialTransparent->apply(cam);
	GLint model_location = glGetUniformLocation(m_materialTransparent->getDefaultProgram(), "model");
	if (model_location != -1) {
...
void Scene::setTransparentColor(int objId, float r, float g, float b, float a) {
	if (objId >= m_meshes.size()||objId<0)
		return;
	m_meshes[objId]->m_materialTransparent->m_diffColor.r = r;
	m_meshes[objId]->m_materialTransparent->m_diffColor.g = g;
	m_meshes[objId]->m_materialTransparent->m_diffColor.b = b;
	m_meshes[objId]->m_materialTransparent->m_diffColor.a = a;
}
...
int Scene::createPBRObj(std::string name, const std::string& abeldo, const std::string& normal,
	...
	pbr_mesh->m_materialTransparent = std::make_shared<MaterialPhong>(m_shaderFolder);
...
void Scene::drawTransparent(Utility::ArcballCamera* cam, int new_program)
{
	// Draw PBR model.
	if (m_drawTransparent)
	for (auto obj : m_meshes)
