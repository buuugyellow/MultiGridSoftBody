...
__global__ void calculateIFAc(float* positions, int* m_tetIndex, float* m_tetInvD3x3, float* m_tetInvD3x4, float* force, float* tetVolumn, int tetNum,
...
void PDSolverData::runCalculateIFAc(float m_volumnStiffness) {
    ...
    calculateTetDG_Test<<<blockNum, threadNum>>>(tetVertPos_d, tetIndex_d, tetInvD3x3_d, tetDG_d, tetNum);
